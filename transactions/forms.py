...
class SaleForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['email'].widget.attrs.update({'class': 'textinput form-control'})
        self.fields['emp_code'].widget.attrs.update({'class': 'textinput form-control', 'maxlength': '4', 'pattern' : '[0-9]{4}', 'title' : 'Numbers only','required': 'true'})

    class Meta:
        model = SaleBill
        fields = ['name', 'phone', 'address', 'email', 'emp_code']
        widgets = {
            'address' : forms.Textarea(
                attrs = {
                    'class' : 'textinput form-control',
